	...
	useEffect(() => {
		...
		const loadTile = async () => {
			try {
				...
				const parsedLayers = Object.entries(tile.layers).map(
					([name, layer]) => ({
						name,
						extent: layer.extent,
				...
				if (mounted) {
					setError(
						err instanceof Error ? err.message : "Failed to parse vector tile",
					);
				}
			} finally {
				if (mounted) {
			...
			{layers.map((layer) => (
				<View key={layer.name} style={styles.layerCard}>
					...
					<Text style={styles.meta}>
						extent {layer.extent} · version {layer.version} ·{" "}
