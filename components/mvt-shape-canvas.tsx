...
`),
	...
	useEffect(() => {
		...
		const loadTile = async () => {
			try {
				...
				if (!response.ok) {
					throw new Error(
						`Failed to fetch ${TILE_URL}: ${response.status} ${response.statusText}`,
					);
				}
				...
				const mainLayerName = tile.layers[PREFERRED_LAYER]
					? PREFERRED_LAYER
					: availableLayers[0];
				...
				if (selectedLayerNames.length === 0) {
					throw new Error(
						`No layers found in tile. Available layers: ${availableLayers.join(", ")}`,
					);
				}
				...
				selectedLayerNames.forEach((selectedLayerName) => {
					...
					for (let index = 0; index < layer.length; index += 1) {
						...
						if (feature.type === 2)
							lineFeatures.push({ geometry, kind, layer: selectedLayerName });
						if (feature.type === 1)
							pointFeatures.push({ geometry, kind, layer: selectedLayerName });
					}
				});
				...
				const renderPolygons = polygonFeatures.map(
					({ geometry, kind, layer }) => ({
				...
				const renderPoints = pointFeatures.flatMap(
					({ geometry, kind, layer }) =>
				...
				if (mounted) {
					setError(
						err instanceof Error ? err.message : "Failed to render tile",
					);
				}
			}
		};
	...
	const earthPolygons = polygons.filter(
		(polygon) => polygon.layer === EARTH_LAYER && polygon.kind === "earth",
	);
	const oceanPolygons = polygons.filter(
	...
	return (
		<View style={styles.container}>
			...
			{layerNames.length > 0 && (
				<Text style={styles.subtitle}>Layers: {layerNames.join(", ")}</Text>
			)}
						...
						earthPolygons.map((item, index) => (
							<Group key={`earth-shader-${index}`} clip={item.path}>
								<Fill>
									<Shader
										source={grassShader}
										uniforms={{ time: waterTime, resolution: [620, 620] }}
									>
										<ImageShader
											image={image}
											fit="cover"
			...
			<Text style={styles.count}>
				{polygons.length} polygon(s), {lines.length} line(s), {points.length}{" "}
				point(s), {earthPolygons.length} earth polygon(s),{" "}
